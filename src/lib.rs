...
impl Visualizer {
	pub fn new(file: File, width: u16, height: u16, pallete: &[u8], options: VisualizerOptions) -> Result<Self, Box<Error>> {
		...
		vis.clear_rect(((0, height - options.info_height + 2), (width, options.info_height-2)), 1);
		Ok(vis)
	}
	...
	pub fn text(&mut self, x: u16, y: u16, text: &str, color: u8, scale: f32) -> ((u16, u16), (u16, u16)) {
		...
		self.info_rects.push(((x, y), (glyphs_width as u16, glyphs_height as u16)));
